...
with st.sidebar:
    ...
    st.download_button("📥 Download Sample CSV",
                       open("/home/claude/portfolio_tracker/sample_transactions.csv").read(),
...
for tab, (port_name, df) in zip(tabs, all_holdings.items()):
    with tab:
        ...
        total_mktval  = df["Market Value (₹)"].sum()
        ...
        c3.metric("Market Value",   f"₹{total_mktval:,.0f}")
        pnl_str = f"₹{total_pnl:,.0f} ({total_pnl_pct:.1f}%)"
        c4.metric("Unrealized P&L", pnl_str, delta=f"{total_pnl_pct:.1f}%",
                  delta_color="normal")
        ...
        styled = display_df.style\
            .applymap(color_pnl, subset=["Unrealized P&L (₹)", "P&L %"])\
