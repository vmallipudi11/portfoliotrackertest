...
def format_inr(value, decimals=0):
    if pd.isna(value):
        return "N/A"
    return f"₹{format_indian_number(value, decimals=decimals)}"
...
def load_transactions_file(uploaded_file, excel_password=""):
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        ...
        type=["csv", "xlsx", "xlsm"],
        help="CSV or Excel file with columns: Portfolio, Date, Ticker, Action, Quantity, Price",
    )
    excel_password = st.text_input(
        "Excel Password (optional)",
        type="password",
        help="Only needed for password-protected Excel files.",
    )
    st.markdown("---")
...
# Load & validate
try:
    df_raw = load_transactions_file(uploaded, excel_password=excel_password)
...
for tab, (port_name, df) in zip(tabs, all_holdings.items()):
    with tab:
        ...
        c2.metric("Total Invested", format_inr(total_invested, decimals=0))
        c3.metric("Market Value", format_inr(total_mktval, decimals=0) if total_mktval else "N/A")
        if total_mktval:
            pnl_str = f"{format_inr(total_pnl, decimals=0)} ({total_pnl_pct:.1f}%)"
        ...
        styled = display_df.style\
            ...
            .format({
                "Quantity": "{:,.0f}",
                "Avg Cost (₹)": lambda x: format_indian_number(x, decimals=2),
                "Total Invested (₹)": lambda x: format_indian_number(x, decimals=2),
                "Current Price (₹)": lambda x: format_indian_number(x, decimals=2),
                "Market Value (₹)": lambda x: format_indian_number(x, decimals=2),
                "Weight %": "{:.2f}%",
                "Unrealized P&L (₹)": lambda x: format_indian_number(x, decimals=2),
            }, na_rep="N/A")\
        ...
        excel_bytes = build_excel({port_name: df})
        safe_port_name = port_name.replace(" ", "_")
        fname = f"{safe_port_name}_holdings_{datetime.today().strftime('%Y%m%d')}.xlsx"
        st.download_button(
            "⬇️ Download Excel",
            data=excel_bytes,
            file_name=fname,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key=f"download_excel_{port_name}",
        )
        st.caption(f"Exports only the {port_name} sheet · Live prices as of now · FIFO cost basis")
